...
function SalesItemsPage() {
    ...
    useEffect(() => {
        ...
        //확인용 로그
        console.log("로컬스토리지 아이템 상태:", allItems.map(i => i.status));
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.statusBox}>
                <button
                    className={`${styles.onSale} ${filter === "판매중" ? styles.active : ""}`}
                    onClick={() => setFilter("판매중")}
                >판매중</button>
                <button
                    className={`${styles.complete} ${filter === "거래완료" ? styles.active : ""}`}
                    ...
                    <>
                        {filteredItems.length === 0 ? (
                            ...
                            filteredItems.map(item => (
                                <div className={`${styles.item} ${item.status === "거래완료" ? styles.disabled : ""}`}
                                     onClick={() => {
                                         if (item.status !== "거래완료") {
